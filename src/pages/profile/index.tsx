    ...
    if (validateProfile()) {
      try {
        ...
        if (response.status === 200 && response.data) {
          ...
          if (updatedUserInfo.profileSetup) {
            navigate("/chat");
          }
        }
      } catch (error) {
          ...
          >
            <Avatar className="h-32 w-32 md:w-48 md:h-48 rounded-full overflow-hidden">
              {image ? (
                ...
                <div
                  className={`uppercase h-32 w-32 md:w-48 md:h-48 text-5xl border flex items-center justify-center rounded-full ${getColors(selectedColor)}`}
                >
                  {firstName
                    ? firstName.charAt(0)
